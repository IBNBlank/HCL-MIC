...
if __name__ == '__main__':
    ...
    if mpi_comm.Get_rank() == 0:
        encoder_pretrained_file = get_pretrained_file([
            "stage_1/encoder/encoder.pth", "stage_0/encoder/encoder.pth",
            "stage_pretrain/encoder/encoder.pth"
        ])
        state_pretrained_file = get_pretrained_file([
            "stage_1/state/state.pth", "stage_0/state/state.pth",
            "stage_pretrain/state/state.pth"
        ])
        actor_pretrained_file = get_pretrained_file(
            ["stage_1/actor/actor.pth", "stage_0/actor/actor.pth"])
        critic_pretrained_file = get_pretrained_file(
            ["stage_1/critic/critic.pth", "stage_0/critic/critic.pth"])

    #### stage1 class ####
    stage1 = Stage1(comm=mpi_comm,
                    reward_gamma=0.0,
