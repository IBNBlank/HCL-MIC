...
public:
    ...
    StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names, bool pause);
    ~StageNode();
    ...
    // Message callback for a pause message, which sets pause_flag
    void pauseReceived(const boost::shared_ptr<std_msgs::Int8 const>& msg);
    inline bool getPause() { return isPause; }
...
void
StageNode::pauseReceived(const boost::shared_ptr<std_msgs::Int8 const>& msg) {
    isPause = msg->data == 0 ? false : true;
}
...
StageNode::poseReceived(int idx, const boost::shared_ptr<geometry_msgs::Pose const>& msg)
{
    ...
    pose.z = 0;
    pose.a = 2.0 * atan2(msg->orientation.z, msg->orientation.w);
...
StageNode::poseStampedReceived(int idx, const boost::shared_ptr<geometry_msgs::PoseStamped const>& msg)
{
    ...
    pose.z = 0;
    pose.a = 2.0 * atan2(msg->pose.orientation.z, msg->pose.orientation.w);
    this->positionmodels[idx]->SetPose(pose);
}

StageNode::StageNode(int argc, char** argv, bool gui, const char* fname, bool use_model_names, bool is_pause)
{
    this->use_model_names = use_model_names;
    this->isPause = is_pause;
...
StageNode::SubscribeModels()
{
    ...
    pause_sub_ = n_.subscribe("/hex_pause", 10, &StageNode::pauseReceived, this);
...
main(int argc, char** argv)
{
    ...
    StageNode sn(argc-1,argv,gui,argv[argc-1], use_model_names, is_pause);
